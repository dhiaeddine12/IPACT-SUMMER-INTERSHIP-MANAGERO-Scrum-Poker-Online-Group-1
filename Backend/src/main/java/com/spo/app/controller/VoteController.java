...
public class VoteController {
    ...
    @GetMapping("/vote_statistics/{issueTitle}/{session_token}")
    public ResponseEntity<?> getVoteStatistics(@PathVariable("issueTitle") String issueTitle,@PathVariable("session_token") String session_token) {
        //List<Vote> votes = voteRepository.findAllByIssueTitle(issueTitle);
        List<Vote> votes = voteRepository.findAllByIssueTitleAndSessionToken(issueTitle,session_token);
    ...
    @GetMapping("/{sessionToken}/all-issues/results")
    public ResponseEntity<Map<String, Map<String, Integer>>> getAllIssuesVoteResultsBySession(@PathVariable String sessionToken) {
        Map<String, Map<String, Integer>> results = voteService.getAllIssuesVoteResultsBySession(sessionToken);
