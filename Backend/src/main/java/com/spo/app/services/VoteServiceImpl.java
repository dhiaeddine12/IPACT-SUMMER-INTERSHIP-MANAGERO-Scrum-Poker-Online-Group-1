...
public class VoteServiceImpl implements  IVoteService{
    ...
    public Map<String, Map<String, Integer>> getAllIssuesVoteResultsBySession(String sessionToken) {
        List<Vote> votes = voteRepository.findAllBySessionToken(sessionToken);
        Map<String, Map<String, Integer>> voteResults = new HashMap<>();

        for (Vote vote : votes) {
            String issueTitle = vote.getIssue().getTitle();
            String voteValue = String.valueOf(vote.getValue());

            voteResults.computeIfAbsent(issueTitle, k -> new HashMap<>())
                    .merge(voteValue, 1, Integer::sum); // Compte les votes
        }
